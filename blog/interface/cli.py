...
from ..infrastructure.notifier import DiscordNotifier
...
def build_service(workspace: Workspace | None = None) -> PipelineService:
    ...
    return PipelineService(
        ...
        notifier=DiscordNotifier(log_path=ws.notify_log),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if cmd == "status":
        ...
        print("⛔ BLOCKED: 'full' command disabled. Use Discord → Orchestrator → Kanban instead.")
        print("   Semua pipeline harus lewat Discord, bukan direct CLI.")
        return 1
    elif cmd == "run-job" and len(rest) >= 2:
        print("⛔ BLOCKED: 'run-job' command disabled. Use Discord → Orchestrator → Kanban instead.")
        print("   Semua pipeline harus lewat Discord, bukan direct CLI.")
        return 1
    elif cmd == "start" and rest:
        print("⛔ BLOCKED: 'start' command disabled. Use Discord → Orchestrator → Kanban instead.")
        print("   Semua pipeline harus lewat Discord, bukan direct CLI.")
        return 1
